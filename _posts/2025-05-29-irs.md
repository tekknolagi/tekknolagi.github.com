...
In a CFG representation, the instructions are already *scheduled*, or ordered.
Normally this comes from the programmer in the original source form and is
faithfully maintained. We get data use edges in an IR like SSA, but the control
information is left implicit. Some forms of IR, however, seek to reify both
data and control dependencies into the IR itself. One such IR design is *sea of
nodes* (SoN), which was originally designed by Cliff Click during his PhD.

TODO

Design also include region value state dependency graphs (RVSDG) and program
