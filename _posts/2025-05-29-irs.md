...
instructions (variables). Consider the following SSA program (represented as
pseudo-Python):
...
This is complicated (we have to replace all uses, we have to maintain SSA, etc)
but gives us new places to store information *inside the IR*. It means that
every time we refer to `newx0`, we know that it is nonnegative and every time
we refer to `newx1`, we know that it is negative. This information is
