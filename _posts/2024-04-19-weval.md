...
In order to specialize the interpreter, we do three steps at initialization time:
...
While the code is constant, weval isn't magic. Not much has changed in our
`ExecuteSpecialized` function. In order to start making things faster, we have
to drop little hints into the interpreter.
...
interpreter into a compiler. There is only one optimization so far---unrolling
the loop---but it's very effective. The result is a fully WebAssembly module
with no interpreter and no bytecode.
...
Our interpreter is tiny and not very interesting on its own. It's only useful
to explain some weval concepts to you. But the same principles apply to much
larger interpreters, too! Chris Fallin has been working on applying these
techniques to SpiderMonkey, Mozilla's JS and Wasm runtime. The same could also
probably be done for CPython, the main Python runtime. CPython even has support
for WebAssembly already!
