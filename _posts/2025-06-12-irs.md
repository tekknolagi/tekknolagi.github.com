...
remain defined. The stack does not exist.
...
refer to one another **by pointer**. Then they directly refer to one another
(no need for a side-table), which might be faster and more compact. We can
re-create nice names as needed for printing.
