...
# For example,
# {0: "navy", 1: "crimson", 2: "darkgreen", 3: "navy", 4: "crimson"}
```
...
```python
# Only prover. Wrong!
def hash_coloring(coloring):
    return {node: hash(color) for node, color in coloring.items()}
# For example,
# {0: -7781689093656095660, 1: -1612937230357218329, 2: 40637533866397718,
#  3: -7781689093656095660, 4: -1612937230357218329}
```
...
`"navy"` maps to `-778...`, `"crimson"` maps to `-1612...`, and therefore that
`"darkgreen"` must map to `4063...`. With that, the verifier has seen through
our veil.
