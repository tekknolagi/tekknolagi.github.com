...
should swap around the color values while maintaining the 3-color property. I
came up with this ugly function:
...
# For example:
# {0: ("crimson", 33), 1: ("navy", 65), 2: ("darkgreen", 62), 3: ("crimson", 51), 4: ("navy", 38)}
hashed_coloring = hash_values(boxed_coloring)
# For example:
# {0: -2275004828450249492, 1: 2227921633151400991, 2: -5024343381376265886, 3: -5381005702768533635, 4: 1164729608819214729}
```

Finally, we can send the `hashed_coloring` to the verifier and begin step 2.
...
```python
# Only verifier
revealed = prover_please_reveal_colors(random.choice(edges))
```
...
gives a small amount of credence that the graph is 3-colored appropriately
because the prover had no way of knowing which edge the verifier would want to
inspect.
